...
const ADMIN_KEY = 'jogpro_admin_2026'
...
export default function AdminAffiliatorPage() {
  ...
  async function fetchAffiliators() {
    const r = await fetch('/api/admin/affiliators?status=pending', { headers: { 'x-admin-key': ADMIN_KEY } })
    setAffiliators(await r.json())
  }

  async function fetchClaims() {
    const r = await fetch('/api/admin/claims?status=pending', { headers: { 'x-admin-key': ADMIN_KEY } })
    setClaims(await r.json())
  }

  async function fetchSummary() {
    const r = await fetch('/api/admin/summary', { headers: { 'x-admin-key': ADMIN_KEY } })
    setSummary(await r.json())
  }

  function login() {
    if (pw === ADMIN_KEY) { sessionStorage.setItem('admin_authed', '1'); setAuthed(true) }
    else setPwError('Password salah')
  }

  async function approve(id: string) {
    setLoadingId(id)
    const r = await fetch('/api/admin/approve', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': ADMIN_KEY }, body: JSON.stringify({ affiliator_id: id }) })
  ...
  async function reject(id: string) {
    ...
    setLoadingId(id)
    await fetch('/api/admin/reject', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': ADMIN_KEY }, body: JSON.stringify({ affiliator_id: id }) })
    setLoadingId(null)
    fetchAffiliators()
  }
  ...
  async function reviewClaim(id: string, action: 'approve' | 'reject') {
    setLoadingId(id)
    await fetch('/api/admin/claim-review', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': ADMIN_KEY }, body: JSON.stringify({ claim_id: id, action }) })
    setLoadingId(null)
    fetchClaims()
  }
  ...
  if (!authed) return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0a0a0a,#1a0533)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, padding: 32, width: '100%', maxWidth: 360 }}>
        ...
        <button onClick={login} style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg,#8B5CF6,#EC4899)', border: 'none', borderRadius: 12, color: '#fff', fontWeight: 700, fontSize: 16, cursor: 'pointer' }}>Masuk</button>
      </div>
    </div>
  )
