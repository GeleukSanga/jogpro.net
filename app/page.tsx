...
import { ArrowRight, Flame, Menu, Pencil, ShoppingBag, Sparkles, Timer, X, Zap } from 'lucide-react'
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#fbfbf8] text-[#171717]">
      ...
      <section id="shop" className="relative z-10 mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10"><div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-xs font-black uppercase tracking-[0.2em] text-[#ef4b32]">The collection</p><h2 className="text-4xl font-black tracking-[-0.06em] sm:text-5xl">Pilih Gaya Kamu.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#666]">Case korek 3D dengan desain unik yang dibuat untuk dipakai setiap hari.</p></div><div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 text-xs font-bold transition ${filter === item ? 'bg-[#171717] text-white' : 'border border-black/10 bg-white text-[#777] hover:border-black/30'}`}>{item}</button>)}</div></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{visibleProducts.map((product) => <article key={product.id} className="product-card group"><div className={`product-image accent-${product.accent}`}><span className="product-tag">{product.tag}</span><img src={product.image} alt={`${product.name}, ${product.kind}`} style={product.objectPosition ? { objectPosition: product.objectPosition } : undefined} /><div className="card-shine" /></div><div className="flex items-start justify-between gap-3 pt-4"><div><p className="mb-1 text-[10px] font-black uppercase tracking-[0.16em] text-[#999]">{product.kind}</p><h3 className="text-lg font-black tracking-[-0.04em]">{product.name}</h3><p className="mt-1 text-xs leading-5 text-[#777]">{product.description}</p></div><div className="whitespace-nowrap text-right">{promoActive ? (<><p className="text-xs text-[#999] line-through">{product.oldPrice}</p><p className="text-sm font-black text-[#ef4b32]">{product.price}</p></> ) : (<p className="text-sm font-black">{product.oldPrice}</p>)}</div></div><button onClick={() => buy(product)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-full border border-black/10 bg-white py-3 text-xs font-black transition hover:bg-[#171717] hover:text-white">{product.kind === 'Custom' ? 'Custom Sekarang' : 'Pilih Punyamu'} <ArrowRight className="size-3.5" /></button></article>)}</div></section>
