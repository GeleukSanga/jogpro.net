...
const products = [
  {
    ...
    kind: 'Case motif',
    price: 'Rp 20.000',
    image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/ChatGPT%20Image%20Sep%205%2C%202026%20at%2009_42_05%20PM-2gUW7il51MvD8mqs27gsvaugVsVJ6C.png',
    tag: 'Best seller',
    accent: 'violet',
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#fbfbf8] text-[#171717]">
      ...
      <section id="top" className="relative z-10 mx-auto grid max-w-7xl items-center gap-10 px-5 pb-20 pt-12 sm:px-8 md:grid-cols-[1fr_0.9fr] md:pb-28 md:pt-20 lg:px-10">
        ...
        <div className="hero-product-wrap"><div className="hero-ring ring-one" /><div className="hero-ring ring-two" /><div className="floating-chip chip-one"><Zap className="size-3 fill-current" /> tactile finish</div><div className="floating-chip chip-two">01 / 04</div><div className="hero-product"><img src={products[0].image} alt="Case Neon Drip dengan korek api" /><div className="shine" /></div></div>
      </section>

      <section id="shop" className="relative z-10 mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10"><div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-xs font-black uppercase tracking-[0.2em] text-[#ef4b32]">The collection</p><h2 className="text-4xl font-black tracking-[-0.06em] sm:text-5xl">Pick your personality.</h2></div><div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 text-xs font-bold transition ${filter === item ? 'bg-[#171717] text-white' : 'border border-black/10 bg-white text-[#777] hover:border-black/30'}`}>{item}</button>)}</div></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{visibleProducts.map((product) => <article key={product.id} className="product-card group"><div className={`product-image accent-${product.accent}`}><span className="product-tag">{product.tag}</span><img src={product.image} alt={`${product.name}, ${product.kind}`} /><div className="card-shine" /></div><div className="flex items-start justify-between gap-3 pt-4"><div><p className="mb-1 text-[10px] font-black uppercase tracking-[0.16em] text-[#999]">{product.kind}</p><h3 className="text-lg font-black tracking-[-0.04em]">{product.name}</h3><p className="mt-1 text-xs leading-5 text-[#777]">{product.description}</p></div><p className="whitespace-nowrap text-sm font-black">{product.price}</p></div><button onClick={() => buy(product)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-full border border-black/10 bg-white py-3 text-xs font-black transition hover:bg-[#171717] hover:text-white">Checkout <ArrowRight className="size-3.5" /></button></article>)}</div></section>
